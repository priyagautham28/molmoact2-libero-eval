...
# Used the same max steps as in the original evaluation script for each suite https://github.com/huggingface/lerobot/blob/main/src/lerobot/envs/libero.py
TASK_MAX_STEPS = {
    "libero_spatial": 280,
    "libero_object": 280,
    "libero_goal":   300,
...
for task_id in range(n_tasks):
    ...
    for episode_idx in range(n_episodes):
        ...
        while not done and step < max_steps:
            ...
            # Language instruction (preprocessor batches str to list)
        ...
        log.info("  ep %3d/%d %s | steps=%d | seed=%d | success %d/%d",
                 episode_idx, n_episodes - 1, status, step,
                 episode_seed(episode_idx), task_successes, episode_idx + 1)
